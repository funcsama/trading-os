...
`covered` 若能可靠估值，给出由当前已验证事实和正常化经营支持的核心合理价值区间及关注价；未兑现交易、战略稀缺性或新业务期权只能在估值说明中单列条件和上修空间，不得与核心区间合并。事实口径、正常化经营、估值参数和决策安全边际必须分层陈述。若无法可靠估值但事件具有持续研究价值，可以只做事件监控，并填写无法估值原因。`ignore` 不得激活价格触发。

### 研究复核门槛

价格线只安排研究复核，不表达买入或仓位。新报告统一使用以下标识，不再创造“高吸引力”“安全边际”“深度价值”等近义字段：

- `attention`（关注复核价）：默认价格线。价格进入该区域后，检查最新事实、核心价值和证伪条件，避免因估值保守而停止观察；
- `deep_review`（深度复核价）：可选的第二价格线。只有更低价格会使研究问题发生实质变化，例如需要穿透悲观情景、资产负债表或现金流支撑时才设置。

不是每家公司都需要两条价格线。只有一个有效复核门槛时只写 `attention`；无法可靠定价时可只保留事件触发。`deep_review` 必须低于 `attention`，命中后仍需重新核实事实和估值，不自动产生资本投入、仓位或组合动作。历史报告中的 `attractive / high_attraction / deep_attention` 等标识继续兼容，等公司因新财报、重大事件或估值失效而更新时再自然统一，不为改名批量重写报告。
...
缺少任意受监控公司报价时整批失败。价格命中不自动交易，也不自动创建完整研究任务。多个价格层级只能表示研究复核的紧迫度差异；`deep_review` 也不代表账户、仓位或组合层面的资本投入授权。
