...
- 安全边际按风险严重度分档：标准档沿用高/中置信度 10%/15%，可量化的重大风险至少 20%，严重且难量化的永久损失风险至少 25%。
- 出现治理重大疑点或周期位置不明时至少使用提高档；存在永久损失风险时必须使用严重档，同时仍需进入 challenger，不能只靠低价放行。
- 同一风险不得无说明地同时通过折现率、悲观情景折价和安全边际重复收费。报告必须说明风险进入估值的具体位置。
- 证据缺失改变的是承保状态，不能用更低价格或更大折扣替代必要证据。
