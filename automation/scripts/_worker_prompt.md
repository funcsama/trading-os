...
Claude Code 的 WebSearch/WebFetch 有时会被域名安全策略限制。遇到这种情况时，不要立刻失败，
必须继续尝试命令行方式获取公开资料：

- 使用 `python` + `requests` / `urllib` 或 `curl` 获取公开网页、JSON API、PDF。
- A 股优先尝试：交易所公告、巨潮资讯、公司官网投资者关系、东方财富/同花顺 F10、证券时报/财联社等公开页面。
- 行情快照可用调度器提供的 `COMPANY_SNAPSHOT_JSON`，但当前价、市值最好再用公开行情源复核。
- 年报/季报无法下载时，可以使用已核验的公告摘要、交易所摘要、公司 IR 摘要和多源交叉验证数据，但必须写明置信度。
- 只有在命令行抓取、公开网页、公司官网和本地快照都无法支持基本面判断时，才写失败报告。
- 单家公司资料收集预算约 20 分钟；不要无限搜索。资料不足时给低置信度结论，而不是卡住。
